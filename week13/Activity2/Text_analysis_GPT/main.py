...
def analyze_cv(cv_text):
    ...
    prompt = f"""
    You are an expert software developer analyzing a candidate's CV in IT, software engineering, data analytics and computer science fields. 

    1. Identify and categorize the candidate's experience into fields (e.g., Software Engineering).
    ...
    try:
        client = genai.Client(api_key="AIzaSyATChOR8JzcFhrThVc7BykER5mmU0-JCw4")
        response = client.models.generate_content(
            model="gemini-2.5-flash", contents=prompt
        )
        print("Gemini response:", response.text)
    except Exception as e:
        print("Error communicating with AI API:", e)
